...
# Dynamically locate source VM details regardless of exact naming (postgres-vm vs postgresql-vm)
...
# Derive region directly from the zone to guarantee it is never empty
export REGION=${ZONE%-*}

# Find destination Cloud SQL instance
...
Pre-attaches the destination instance to the default VPC to prevent Database Migration Service setup hangs.
...
Installs pglogical, appends required configurations, sets output_plugin_libraries, creates the replication user, and adds the missing primary key on inventory_items.
...
Creates the source connection profile pointing directly to the discovered internal IP.
...
CHECK PROGRESS: Check Task 1 once the job status shows Starting or Running.
...
Enables IAM auth on Cloud SQL and assigns the required table permissions.
...
Enables log retention, writes a test row, and provisions a PITR clone.
